...
public class RangeReportController {
    ...
    private static XSSFCellStyle style(XSSFWorkbook w,String bg,String fg,boolean bold,int size){XSSFCellStyle s=w.createCellStyle();s.setFillForegroundColor(new org.apache.poi.xssf.usermodel.XSSFColor(Color.decode("#"+bg),null));s.setFillPattern(FillPatternType.SOLID_FOREGROUND);org.apache.poi.xssf.usermodel.XSSForg.apache.poi.xssf.usermodel.XSSFFont f=w.createFont();f.setColor(new org.apache.poi.xssf.usermodel.XSSFColor(Color.decode("#"+fg),null));f.setBold(bold);f.setFontHeightInPoints((short)size);f.setFontName("Aptos");s.setFont(f);s.setVerticalAlignment(VerticalAlignment.CENTER);return s;}
