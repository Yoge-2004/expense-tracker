...
def split_dataset(
    ...
    if len(india) >= 20:
        india_train, india_holdout = train_test_split(
            india,
            test_size=0.20,
            random_state=seed,
            stratify=india["label"] if india["label"].value_counts().min() >= 2 else None,
        )
        general = pd.concat([general, india_train], ignore_index=True)
    else:
        india_holdout = _empty_like(india)

    # Tiny synthetic/smoke datasets cannot satisfy meaningful train/test
    # partition sizes. Keep all rows in train so callers can still exercise
    # the pipeline and artifact generation without fabricating an evaluation set.
    if len(general) < 5:
        train = general.drop(columns="_text_hash").reset_index(drop=True)
        return SplitResult(train, _empty_like(train), _empty_like(train), india_holdout.drop(columns="_text_hash").reset_index(drop=True))

    # The prepared dataset already removes exact duplicate text+label rows.
    # Hashes remain available for callers that need leakage diagnostics.
    general = general.drop(columns="_text_hash")
    ...
    return SplitResult(
        ...
        india_holdout.drop(columns="_text_hash", errors="ignore").reset_index(drop=True),
    )
