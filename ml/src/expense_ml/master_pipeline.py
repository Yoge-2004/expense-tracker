...
def train_all(frame: pd.DataFrame, cfg: TrainingConfig, run_dir: Path, include_transformer: bool = True) -> dict:
    ...
    try:
        merchant = MerchantSimilarityIndex.fit(frame["text"].tolist())
        merchant.save(models_dir / "merchant-similarity")
        write_json(
            {"unique_merchants": len(merchant.merchants)},
            run_dir / "reports" / "merchant_index.json",
        )
    ...
    try:
        duplicate = DuplicateSimilarityModel.fit(frame["text"].tolist())
        pairs = duplicate.duplicate_pairs()
        duplicate.save(models_dir / "duplicate-similarity")
        write_json(
            {"candidate_pairs": len(pairs), "threshold": 0.92, "examples": pairs[:1000]},
    ...
    write_json(
        {"models": results, "selection_metric": "macro_f1"},
        run_dir / "reports" / "model_comparison.json",
    )
