...
class TransformerCategoryModel:
    ...
    def build(cls, model_name: str, labels: list[str], max_length: int = 96):
        ...
        model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=len(labels), label2id=label2id, id2label=id2label)
    ...
    def predict(self, texts, top_n: int = 3) -> PredictionBatch:
        ...
        self.model.eval()
        inputs = self.tokenizer(list(texts), padding=True, truncation=True, max_length=self.max_length, return_tensors="pt")
        with torch.no_grad():
    ...
    def save(self, path: Path) -> None:
        ...
        (path / "metadata.json").write_text(json.dumps({"model_type": "transformer", "labels": self.labels, "max_length": self.max_length}, indent=2), encoding="utf-8")
    ...
    def load(cls, path: Path):
        ...
        return cls(AutoModelForSequenceClassification.from_pretrained(path), AutoTokenizer.from_pretrained(path), metadata["labels"], metadata["max_length"])
